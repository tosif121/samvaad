...
class OemOptimizationService {
  ...
  /// Returns whether "Display over other apps / Appear on top" is granted.
  Future<bool> isOverlayPermissionGranted() async {
    try {
      final granted = await _channel.invokeMethod<bool>('getOverlayPermissionStatus');
  ...
  /// Opens the OEM-specific "Appear on top" settings screen for this device
  /// and returns whether the permission is now granted.
  Future<bool> openOverlayPermission() async {
    try {
      final granted = await _channel.invokeMethod<bool>('openOverlayPermission');
      return granted ?? false;
    } catch (_) {
      return false;
    }
  ...
  /// Opens the device's "Full screen" (allow full-screen notifications / show
  /// on lock screen) settings screen and returns whether it was opened.
  Future<bool> openFullScreenPermission() async {
    try {
      final opened = await _channel.invokeMethod<bool>('openFullScreenPermission');
      return opened ?? false;
    } catch (_) {
      return false;
    }
  ...
  Future<void> checkAndShowOemGuidanceDialog(BuildContext context) async {
    ...
    // 2. Check if OEM autostart guidance should be shown
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              _buildStepItem('2. Allow "Full screen" so calls show when locked.'),
          ...
          actions: [
            ...
            TextButton.icon(
              ...
              label: const Text('Appear on Top'),
            ),
            TextButton.icon(
              onPressed: () async {
                await markOemGuidancePrompted();
                if (ctx.mounted) Navigator.of(ctx).pop();
                await openFullScreenPermission();
              },
